...
class DBank extends Sign
{
	...
	{
		//序列化 cookie 文件路径
		$sercookieFile = $this->cookiePath.$this->cookieName.'.sercookie';

		//序列化文件存在直接读取，否则重新登录
		if(file_exists($sercookieFile))
		{
			$sercookie = file_get_contents($sercookieFile);
		...
		else
		{
			...
			$kResponse = $this->POSTRequest($kUrl, http_build_query($loginData), $httpheader);
			$kResponse = json_decode($kResponse);

			//访问验证地址，获得 cookie
			$authUrl = $this->loginUrl.'k='.$kResponse->k;
			...
			//序列化 cookie 值
			file_put_contents($sercookieFile, serialize($cookies));
		}
	...
	private function signThis($signParams, $secretKey)
	{
		...
		//执行请求
		$signResponse = $this->GETRequest($signUrl);
		$signResponse = json_decode($signResponse);

		//处理请求结果
		if(isset($signResponse->retdesc))	
			$this->logString .= $signResponse->retdesc;
		elseif(isset($signResponse->error))
			$this->logString .= $signResponse->error;
		else
			$this->retry($sercookieFile);
